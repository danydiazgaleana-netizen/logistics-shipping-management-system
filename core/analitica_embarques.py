...
def cargar_y_validar_datos(ruta_csv):
    ...
    # Estandarizar nombres de columnas por si acaso vienen con mayúsculas/minúsculas distintas
...
def predecir_riesgo_retraso(df):
    ...
    # Condición analítica de riesgo basada en datos limpios
    ...
    for index, row in df.iterrows():
        print(f"OV: {row['OV']} | Destino: {row['Ubicación']} | Paquetería: {row['Paquetería']} -> Alerta: {row['Riesgo_Prediccion']}")
        
    return df

def analizar_embarques(ruta_csv):
    # 1. Cargar y validar usando la función robusta
    df = cargar_y_validar_datos(ruta_csv)
    
    print("--- REPORTE DE ANALÍTICA DE EMBARQUES ---")
    print(f"Total de embarques procesados: {len(df)}\n")
    
    # 2. Promedio de días de estancia (Usando el nombre exacto de la columna validada)
...
if __name__ == "__main__":
    # Ruta del archivo dummy en el proyecto
